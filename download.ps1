...
# Четко фиксируем одну целевую папку
$targetFolder = "C:\ss"

# Создаем папку, если её нет, и проверяем её доступность
...
$files = @(
    ...
    [PSCustomObject]@{ Name = "RSS AmCache"; FileName = "RSS-AmCache.exe"; URL = "https://cdn.discordapp.com/attachments/1495118516657848420/1505148397164826705/RSS-AmCache.exe?ex=6a09922e&is=6a0840ae&hm=e79dfcc3d5a1b64f2414df9fe156a96f7fb2b0a438efb9d1ed89cc2e8dbb6475&" }
    ...
    [PSCustomObject]@{ Name = "DIE x86 (ZIP)"; FileName = "die_win32_portable_3.22_x86.zip"; URL = "https://github.com/horsicq/DIE-engine/releases/download/3.22/die_win32_portable_3.22_x86.zip" }
    [PSCustomObject]@{ Name = "DIE x64 (ZIP)"; FileName = "die_win64_portable_3.22_x64.zip"; URL = "https://github.com/horsicq/DIE-engine/releases/download/3.22/die_win64_portable_3.22_x64.zip" }
...
# Функция отрисовки меню (Только чистый список)
...
foreach ($f in $files) {
    ...
    # Безопасное удаление старого файла перед записью
    ...
    try {
        ...
        # Скачиваем файл напрямую в целевую папку C:\ss
